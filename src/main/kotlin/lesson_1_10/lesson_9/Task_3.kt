...
fun main() {
    val egg = "Яйцо"
    val milk = "Молоко"
    val butter = "Сливочное масло"
    val quantityIngredients = mutableListOf(2, 50, 15)
    var numberOfServings = 0

    print("Сколько порций омлета Вам требуется? >> ")
    numberOfServings = readln().toInt()
    val resultIngredients = quantityIngredients.map {
        it * numberOfServings
    }
    println(
        "На $numberOfServings порции вам понадобится: \n" +
                "$egg – ${resultIngredients[0]} шт.\n" +
                "$milk – ${resultIngredients[1]} мл.\n" +
                "$butter – ${resultIngredients[2]} гр."
    )
}